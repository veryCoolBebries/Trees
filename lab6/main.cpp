...
int main() {
    ...
    addNewButton_texture.loadFromFile("/resources/addNewNode.png");
    Texture deleteButton_texture;
    deleteButton_texture.loadFromFile("/resources/deleteNode.png");
    Texture searchButton_texture;
    searchButton_texture.loadFromFile("/resources/searchNode.png");
    Texture textField_texture;
    textField_texture.loadFromFile("/resources/textField.png");
    Font montserratBold;
    if (!montserratBold.loadFromFile("/resources/MontserratBold.ttf")) {
        throw exception("FONT_NOT_LOADED");
    }
    //Создаём дерево
    srand(std::time(NULL));
    insert(myTree, 0);
        ...
        while (myWindow.pollEvent(event))
        {
            ...
            case sf::Event::MouseButtonPressed:
                if (addNewButton.isMouseOver(myWindow)) {
                    ...
                    catch (exception& error) { //TODO: Починить отлов ошибки
